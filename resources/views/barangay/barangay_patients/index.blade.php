            ...
            <div class="d-flex">
                @if (auth()->user()->isBHW())
                    ...
                    <button type="button" class="btn btn-success" data-toggle="modal"
                        data-target="#generateBarangayPatientReportModal">
                        <i class="fas fa-file-export"></i> Report
                    </button>
                @endif
            </div>
        </div>
        <div class="modal fade" id="generateBarangayPatientReportModal" tabindex="-1" role="dialog"
            aria-labelledby="generateBarangayPatientReportModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="generateBarangayPatientReportModalLabel">Generate Report</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
                    <div class="modal-body">
                        <form action="{{ route('barangay-patients.generateBarangayPatientReport') }}" method="post">
                            @csrf
                        ...
                        <table class="table table-hover table-sm">
                            ...
                            <tbody>
                                @foreach ($barangayPatients as $barangayPatient)
                                    ...
                                    <!-- Show Patient Modal -->
                                    @include('barangay.barangay_patients.show_modal', [
                                        'barangayPatient' => $barangayPatient,
                                    ])

                                    <!-- Edit Patient Modal -->
                                    @include('barangay.barangay_patients.edit_modal', [
                                        'barangayPatient' => $barangayPatient,
                                    ])

                                    <!-- Delete Patient Modal -->
                                    @include('barangay.barangay_patients.delete_modal', [
                                        'barangayPatient' => $barangayPatient,
                                    ])
                                @endforeach
                            </tbody>
        ...
        <div class="my-4 text-muted">
            ...
            <div class="float-right">
                ...
                <ul class="pagination">
                    ...
                    @php
                        $currentPage = $barangayPatients->currentPage();
                        $lastPage = $barangayPatients->lastPage();
                        $showFirstDots = false;
                        $showLastDots = false;
                    ...
                    @for ($i = $startPage; $i <= $endPage; $i++)
                        <li class="page-item {{ $i == $currentPage ? 'active' : '' }}">
                            <a class="page-link"
                                href="{{ $barangayPatients->url($i) }}&entries={{ $entries }}">{{ $i }}</a>
                        </li>
                    @endfor
                    ...
                    @if ($showLastDots)
                        ...
                        <li class="page-item">
                            <a class="page-link"
                                href="{{ $barangayPatients->url($lastPage) }}&entries={{ $entries }}">{{ $lastPage }}</a>
                        </li>
                    @endif

                    <li class="page-item {{ $barangayPatients->currentPage() == $lastPage ? 'disabled' : '' }}">
                        <a class="page-link" href="{{ $barangayPatients->nextPageUrl() }}&entries={{ $entries }}"
                            aria-label="Next">
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateTable() {
                ...
                $.ajax({
                    url: "{{ route('barangay-patients.index') }}",
                    type: 'GET',
                    data: {
                        entries: entries,
                        search: searchQuery,
        ...
        function handleSort(column) {
            ...
            window.location = "{{ route('barangay-patients.index') }}?column=" + column + "&order=" + order + "&entries=" +
                entries;
        }
    </script>
    <style>
        ...
        body.dark-mode #generateBarangayPatientReportModal .modal-content {
            background-color: #343a40;
            color: #fff;
        }

        body.dark-mode #generateBarangayPatientReportModal .modal-header {
            background-color: #343a40;
            color: #fff;
            border-bottom: 1px solid #007bff;
        }

        body.dark-mode #generateBarangayPatientReportModal .modal-title {
            color: #fff;
        }

        body.dark-mode #generateBarangayPatientReportModal .modal-body {
            background-color: #343a40;
            color: #fff;
        }

        body.dark-mode #generateBarangayPatientReportModal label,
        body.dark-mode #generateBarangayPatientReportModal .form-control {
            color: #fff;
        }
        ...
        body.dark-mode #generateBarangayPatientReportModal .modal-footer {
            background-color: #343a40;
            border-top: 1px solid #007bff;
        }

        body.dark-mode #generateBarangayPatientReportModal .btn-secondary {
            color: #fff;
        }
    </style>
@endsection
